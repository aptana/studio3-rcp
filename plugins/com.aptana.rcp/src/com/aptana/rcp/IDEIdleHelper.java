 ...
 * algorithm for determining when to perform a garbage collection is as follows:
 * 
 * - Never gc if there is a test harness present - Don't gc if background jobs
 * are running - Don't gc if the keyboard or mouse have been active within
 * IDLE_INTERVAL - Don't gc if there has been a GC within the minimum gc
 * interval (system property PROP_GC_INTERVAL) - After a gc, don't gc again
 * until (duration * GC_DELAY_MULTIPLIER) has elapsed. For example, if a GC
 * takes 100ms and the multiplier is 60, don't gc for at least five seconds -
...
class IDEIdleHelper {
    ...
    /**
     * The default maximum duration for a garbage collection, beyond which the
     * explicit gc mechanism is automatically disabled.
     */
    ...
    /**
     * The maximum gc duration. If this value is exceeded, the entire explicit
     * gc mechanism is disabled.
     */
    ...
    IDEIdleHelper(IWorkbenchConfigurer aConfigurer) {
        ...
        // don't gc while running tests because performance tests are sensitive
        // to timing (see bug 121562)
        ...
        if (enabled != null
                && enabled.equalsIgnoreCase(Boolean.FALSE.toString())) {
            return;
        }
        // init gc interval
        ...
        handler = new Runnable() {
            public void run() {
                if (!display.isDisposed()
                        && !configurer.getWorkbench().isClosing()) {
                    int nextInterval;
                    ...
                    if (!Job.getJobManager().isIdle()) {
                        ...
                        // don't garbage collect if we have collected within the
                        // specific interval
            ...
            protected IStatus run(IProgressMonitor monitor) {
                ...
                if (display != null && !display.isDisposed()) {
                    ...
                    if (duration > maxGC) {
                        ...
                        // if the gc took a long time, ensure the next gc
                        // doesn't happen for awhile
                        nextGCInterval = Math.max(minGCInterval,
                                GC_DELAY_MULTIPLIER * duration);
                        if (Policy.DEBUG_GC) {
                            System.out
                                    .println("Next GC to run in: " + nextGCInterval); //$NON-NLS-1$
                        }
                    }
                }
