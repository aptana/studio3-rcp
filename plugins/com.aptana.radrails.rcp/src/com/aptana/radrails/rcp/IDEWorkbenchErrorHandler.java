...
/**
 * This is the IDE workbench error handler. The instance of this handler is returned from
 * IDEWorkbenchAdvisor#getWorkbenchErrorHandler(). All handled statuses are checked against severity and logged using
 * logging facility (by superclass).
 * 
 * @since 3.3
 */
...
public class IDEWorkbenchErrorHandler extends WorkbenchErrorHandler
{
	...
	/*
	 * (non-Javadoc)
	 * @see org.eclipse.ui.statushandlers.WorkbenchErrorHandler#handle(org.eclipse.ui.statushandlers.StatusAdapter, int)
	 */
	public void handle(final StatusAdapter statusAdapter, int style)
	{
		...
		else if (!isIgnored(statusAdapter))
		{
		...
		if (isFatal(statusAdapter))
		{
			UIJob handlingExceptionJob = new UIJob("IDE Exception Handler") //$NON-NLS-1$
			{
				...
				public IStatus runInUIThread(IProgressMonitor monitor)
				{
					...
					return new Status(IStatus.OK, IdePlugin.PLUGIN_ID,
	...
	private static boolean isIgnored(StatusAdapter statusAdapter)
	{
	...
	/**
	 * Informs the user about a fatal error. Returns true if the user decide to exit workbench or if another fatal error
	 * happens while reporting it.
	 */
	private boolean openQuestionDialog(Throwable t)
	{
		try
		{
			...
			dialog = openInternalQuestionDialog(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(),
					MSG_FATAL_ERROR_Title, MessageFormat.format(MSG_FATAL_ERROR, new Object[] { msg }), null, 1);
