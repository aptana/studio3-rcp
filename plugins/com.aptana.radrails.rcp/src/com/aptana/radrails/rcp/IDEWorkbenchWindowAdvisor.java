...
public class IDEWorkbenchWindowAdvisor extends WorkbenchWindowAdvisor {
     ...
     * opening files dragged in from the windows shell. This logic orginates
     * from NavigatorDragDropAdapter and OpenExtenalFileAction.
     * 
    ...
    private void openWelcomeEditors(IWorkbenchWindow window) {
        if (IDEWorkbenchPlugin.getDefault().getPreferenceStore().getBoolean(
            ...
            Map<String, AboutInfo> groups = wbAdvisor
                    .getNewlyAddedBundleGroups();
            for (Iterator<AboutInfo> it = groups.values().iterator(); it
                    .hasNext();) {
